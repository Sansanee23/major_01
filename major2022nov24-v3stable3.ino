...
#define DEV_ver "24nov2022-v3-stable2"
#define INT_CH1       5   // Y
#define INT_CH2       17  // Z
#define INT_CH3       18  // X
#define INT_CH4       19  // reserve
#define INT_blue_SW   12  //12 mar
#define INT_yellow_SW 27
#define INT_white_SW  25
#define blue_led      13
...
void yellow_led_SW () {
  if (digitalRead(INT_yellow_SW) == 0 ) {
    read_no_pulse3();
    ledState = 0;
    ButtonState = 1;
    SubState = 2;
...
void blue_led_SW () {
  if (digitalRead(INT_blue_SW) == 0 ) {
    if(sendURL==0)
      sprintf( buff , "https://bunnam.com/projects/majorette_pp/update/touch-reset.php?id_mc=%s", readmachine.c_str());
    else
      sprintf( buff , "http://192.168.40.21/majorette_pp/update/touch-reset.php?id_mc=%s", readmachine.c_str());
/*    
    #ifdef URL_BUNNAM
      sprintf( buff , "https://bunnam.com/projects/majorette_pp/update/touch-reset.php?id_mc=%s", readmachine.c_str());
    #else
...
void setup() {
  ...
  //attachInterrupt(digitalPinToInterrupt(INT_CH1), countUpCH1, HIGH );
...
//12mar
  ...
  pinMode(INT_white_SW, INPUT_PULLUP);
  pinMode(INT_blue_SW, INPUT_PULLUP);
  pinMode(INT_yellow_SW, INPUT_PULLUP);
//13sep2022
  //xTaskCreatePinnedToCore(count_sender, "count_sender", 10000, NULL, 1, NULL, 0);
//13sep2022
  //  attachInterrupt(digitalPinToInterrupt(INT_white_SW), white_led_SW, FALLING );
  //  attachInterrupt(digitalPinToInterrupt(INT_blue_SW), blue_led_SW, FALLING );
  //  attachInterrupt(digitalPinToInterrupt(INT_yellow_SW), yellow_led_SW, FALLING );
//12mar
  //--- timer
  ...
  Serial.print("SSID: ");     Serial.println(readssid);
  Serial.print("Password: "); Serial.println(readpassword);
  ssid = readssid.c_str();
        ...
        if (customKey != NO_KEY && key == 1) {
          ...
          if (keystate == 1) {
            ...
            if (key == 1) {
              ...
              key = 0, con = 0, set = 1;
              //ssid = "";  password = ""; pass = "";
              Serial.println("log_mac_address_when_start : WiFi connected");
              delay(5000);  //delay for wait wifi connected succssful
              log_mac_address_when_start(2); 
              display.clear();
              display.resetDisplay();
            }
          }
...
// show qty and cont
    if (confirmRF == 1 && numrole == 1) {
      ...
      if (RFstate == 1) {
        ...
        dw_font_print(&myfont, buff1);
        EEPROM.get(addr_ee_count, readcount);  //  readcount = count upload
        EEPROM.get(addr_ee_qty, readqty);      //  readqty = count pump
/* 27mar2023
        white_led_SW ();
        yellow_led_SW ();
        blue_led_SW ();
*/        
        if (ButtonState == 1)
          delay(500);
        if (ButtonState == 1) {
          if (((Qty_MC > 0) && (SubState == 1)) || ((Qty_MC >= 5) && (SubState == 2)) || ((Qty_MC >= 10) && (SubState == 3))) {
            ...
            Qty_MC = readqty + Sub;
            f_multiplier = Qty_MC * atof(dst.multiplier);
            ButtonState = 0;
            SubState = 0;
...
//button test//
        f_multiplier = Qty_MC * atof(dst.multiplier);
        sprintf( buff1 , "%sC-Qty: %.1f /%s ", dst.op_color, f_multiplier, dst.qty_order);
        dw_font_print(&myfont, buff1);
        //pp_log_display();
        display.display();
//&&&
        ...
        display.display();
//------------------------------------------
        EEPROM.put(addr_ee_qty, readqty);
        y_read = digitalRead(INT_CH1);
        z_read = digitalRead(INT_CH2);
        x_read = digitalRead(INT_CH3);
        result = (y_read * 4) + (z_read * 2) + x_read;
        ...
        */
        if (flag1 != 0) {      // มี interrupt เข้ามา
          ...
          if (chack == 1) {
            ...
            if (!strcmp(dst.op_color, "2")) {
              //Serial.println(" ========= 2C");
              prog_qty_2c();
            }
            if (!strcmp(dst.op_color, "1")) {
              //Serial.println(" ========= 1C");
              prog_qty_1c();
            }
            EEPROM.put(addr_ee_qty, dataqty);
...
/* DB4 */
              ...
              detachInterrupt(digitalPinToInterrupt(INT_CH3));
              detachInterrupt(digitalPinToInterrupt(INT_CH4));
            }
//pause
            else if (customKey1 == '*') {
              customKey1 = NO_KEY;
...
*/
          ...
          display.drawString(6, LINE3, "->  _ _ _");
          display.display();
          /*
            //u
            1  18  +9
...
//09nov2022 - tech: change dt to activity          
          ...
          star_line2();
          Serial.print("dst.id_task                 : ");    Serial.println(dst.id_task);
          Serial.print("(char*)readmachine.c_str()  : ");    Serial.println((char*)readmachine.c_str());
          Serial.print("dst.id_staff                : ");    Serial.println(dst.id_staff);
          Serial.print("dst.activity_type           : ");    Serial.println(dst.activity_type);  
          Serial.print("dst.code_downtime           : ");    Serial.println(dst.code_downtime);          
          query_Activity_GetMethod(dst.id_task, (char*)readmachine.c_str(), dst.id_staff, dst.activity_type, dst.code_downtime, &dst);
          star_line2();
...
int query_quit_dt_op_GetMethod(char* id_activity, char * activity_type, char* id_rfid, char* id_machine , char  * no_send , char * no_pulse1 , char * no_pulse2 , char * no_pulse3 , char * multiplier, char * code_downtime) {
  ...
  //13sep2022
/*  
  Serial.println("==============================quit dt op");
  Serial.println("id_activity   : "+String(id_activity));
  Serial.print("activity_type : ");  Serial.println(activity_type);
  Serial.println("id_rfid       : "+String(id_rfid));
  Serial.println("id_mc         : "+String(id_machine));
  Serial.println("no_send       : "+String(no_send));
  Serial.println("no_pulse1     : "+String(no_pulse1));
  Serial.println("no_pulse2     : "+String(no_pulse2));
  Serial.println("no_pulse3     : "+String(no_pulse3));
  Serial.println("multiplier    : "+String(multiplier));
  Serial.println("code_downtime : "+String(code_downtime));
  Serial.println("==============================");
*/
if(sendURL==0)
